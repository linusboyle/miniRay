...
namespace graphics {
    ...
    RGBColor ShadingPolicy::Lambertian
    (RGBColor cDiffuse, coordinate_type intensity, Vector3 uNormal, Vector3 lDirection) {
        return std::max(0.0, scalarProduct(uNormal,normalize(lDirection)))
                * intensity
                * cDiffuse;
    }

    RGBColor ShadingPolicy::BlinnPhong
    (RGBColor cSpecular, coordinate_type intensity, Vector3 uNormal, Vector3 lDirection,
     Vector3 vDirection, coordinate_type phongExponent) {
        ...
        return std::max(0.0, std::pow(scalarProduct(uNormal, h), phongExponent))
                * intensity
                * cSpecular;
    }
